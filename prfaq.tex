...
\label{faq:need-b}
  ...
  You can also let an agent draft it, which is how most teams author their sixth machine. The drafting layer is z-spec, Punt Labs' free formal-specification toolkit for Z and the B-Method \cite{zspec2026}. It ships as a CLI, an MCP server, and an optional Claude Code plugin, and a CLI-only install covers any other harness. That MCP server is a design-time authoring surface; circuit's own runtime still exposes none (\faqref{faq:not-building}). You describe the workflow in English, and z-spec's \mbox{\texttt{b-create}} drafts the machine in the strict Circuit-B profile (\featureref{feat:zspec-profile}). The five shipped machines serve as its patterns. Companion commands close the loop: \mbox{\texttt{b-check}} type-checks the draft with probcli, \mbox{\texttt{b-animate}} animates and model-checks it, and \mbox{\texttt{b-refine}} handles refinements. ProB verifies that draft exactly as it verifies the shipped five, so a drafting error fails the model check at design time. You then read the machine and approve it before it gates anything, the same review any other committed file gets. The strict profile is what z-spec emits and what the runtime reads, so a drafted machine takes no separate path through the verifier. Agent-authored check bindings are a different question, and \faqref{faq:technical-risks} answers that one.
\end{faqpair}
...
\label{faq:next-step}
  Interview ten engineering teams whose repositories already gate merges on CI. Screen for both segments: teams that already leave agent loops running unwatched, and teams that run agents daily and supervise every run. Count the screening itself. How many teams we contact to reach ten, and how they divide between the two segments, is the first measurement of the segment size left unmeasured in \faqref{faq:tam}. From the already-unattended teams, count how many have built ad-hoc gating scripts around their agents; an existing workaround a team maintains is the strongest demand signal available. From the supervising teams, ask one question: what would you need to see before you stopped watching a run? An answer circuit already produces is a conversion path, and an answer no command can supply kills the framing. Each interview also carries a timed authoring exercise on a workflow the team names. The engineer attempts the machine twice, in two arms. One arm writes raw Circuit-B unassisted, working from the profile documentation and the shipped machines. The other arm gives z-spec's \mbox{\texttt{b-create}} an English description of the same workflow, then reviews the draft it returns. Both arms are timed to a passing probcli model check, so an attempt that never verifies records no finish time. We also record what the reviewer changed in the drafted machine. The Makefile claim in \faqref{faq:need-b} survives only if the unassisted attempt finishes and verifies. The assisted attempt tests the authoring loop we intend teams to use, and a draft the reviewer cannot follow fails it as surely as a draft ProB rejects. In parallel, run pr-watch and tdd-flow on every Punt Labs repository for one month and publish the traces, including every refused transition. The dogfood month also tests the z-spec-to-circuit handoff end to end. We draft a new machine with \mbox{\texttt{b-create}} from an English description, bring it to the strict Circuit-B profile, then load it and drive it in circuit. We count the manual fixes that handoff needs. That count is the acceptance evidence for \featureref{feat:zspec-profile}: many hand fixes make the case for building the profile target, and none retires it. Like the authoring exercise, this measures whether the tooling path works, never whether anyone wants it. If no team builds workarounds, no supervising team names a condition a gate can supply, and our own traces show no interceptions, the value hypothesis fails and we stop. The dogfood month also logs every override, because a team that adopts the tool and then disables it is the third failure mode (\faqref{faq:false-refusals}). None of this gates the release: the repository is public and MIT-licensed now, and publishing first is deliberate (\faqref{faq:competitive}). What the interviews and the traces gate is further investment. A month with no workaround-builders, no interceptions, and no sixth authored machine stops the adapter and embedding work in \faqref{faq:timeline}. The published engine stays MIT either way.
\end{faqpair}
...
\label{faq:technical-risks}
  ...
  Fifth, the authoring layer's build status. The launch path in \faqref{faq:need-b} assumes z-spec's B commands are stable and emit the strict profile, the same assumption structure as the provenance guard above. The Z side of z-spec is settled inside Punt Labs: 57 real specification files across eight sibling repositories, in the low forties counted as distinct subsystems \cite{zspecprfaq2026}. The Koch Trainer iOS app was specified before implementation, with ProB validating its state machine first, and several lux specifications carry deliberate falsification variants. Three of the largest consumers made the practice mandatory in July 2026. The toolkit ships at v0.20.2 with 21 commands on PyPI, over verifiers that are decades old: fuzz since 1988, ProB since 2003. The B commands are the young part. They ship as alpha, working with probcli and untested across a wide range of specifications, and B maturation is a named roadmap item at z-spec. Two measurements in \faqref{faq:next-step} close the distance to the launch assumption: the comparative authoring exercise, and the handoff count behind \featureref{feat:zspec-profile}. Neither measures demand for circuit. Both measure whether the authoring path is practical and cheap.
\end{faqpair}

\begin{faqpair}{What dependencies exist, and what is the fallback?}
\label{faq:dependencies}
  ProB is a development and release dependency only, pinned at 1.15.1 through Nix; runtime needs nothing beyond the Go binary. The z-spec toolkit sits in that same tier. It installs fuzz and ProB, checks our own design spec, and drafts machines for review (\faqref{faq:need-b}), and no runtime path touches it. ProB publishes no Linux aarch64 build, so machine verification runs on x86\_64 in CI. Drive mode currently depends on pi's RPC mode, so pi is the one harness where the unattended path runs today. The hosting relationship works everywhere: any coding harness that can run a command-line tool hosts circuit in tool-call mode, with gates intact and the loop still its own (\faqref{faq:what}).
\end{faqpair}
...
Usability  & \textcolor{RiskAmber}{MEDIUM} & Using a shipped machine is a three-step start. Authoring a new machine needs B literacy or an agent drafting it for human review (\faqref{faq:need-b}), and we have not tested that loop outside Punt Labs. The mitigation under test is z-spec's agent-drafted authoring, settled on the Z side across eight Punt Labs repositories and shipped alpha on the B side (\faqref{faq:technical-risks}). The comparative exercise in \faqref{faq:next-step} decides whether it collapses the B-literacy gap for a team outside Punt Labs. No analogous product gives customers a mental model. \\
...
\begin{enumerate}[nosep,leftmargin=2.5em]
  ...
  \featureitem{circuit profile in z-spec}{a circuit-profile target for z-spec's \mbox{\texttt{b-create}}, emitting strict Circuit-B plus \mbox{\texttt{.checks.yaml}} stubs, so a drafted machine lands inside the profile the runtime accepts (\faqref{faq:need-b})}\label{feat:zspec-profile}
\end{enumerate}
