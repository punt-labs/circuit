...
\begin{zed}
  STOPRESULT ::= stopOK | stopNoActiveSession | stopAmbiguousSession |
    stopUnknownSession
\end{zed}
...
\begin{schema}{StopNoActiveSession}
  \Xi Circuit \\
  result! : STOPRESULT
\where
  active = \emptyset \\
  result! = stopNoActiveSession
\end{schema}
...
\begin{zed}
  StopImplicit \defs StopImplicitOK \lor StopNoActiveSession \lor StopAmbiguousSession
\end{zed}
...
\begin{zed}
  StopTargeted \defs StopTargetedOK \lor StopTargetedUnknown
\end{zed}
...
This first draft is intended to type-check with fuzz:

\begin{verbatim}
fuzz -t docs/spec/circuit-runtime.tex
\end{verbatim}

The first ProB run uses one machine, one session, and one check. That scope is
small, but it is enough to exercise every public operation branch and the
load/scaffold/start interplay around one BOOL fact:

\begin{verbatim}
probcli docs/spec/circuit-runtime.tex -model_check \\
  -p DEFAULT_SETSIZE 1 -p MAX_INITIALISATIONS 200
\end{verbatim}
...
\begin{itemize}
  ...
  \item 77 states analysed;
  \item 778 transitions fired;
  \item no counterexample found;
  \item all open states visited.
\end{itemize}

A wider two-element carrier currently requires a higher initialization budget,
because the initial machine/check configuration is intentionally unconstrained
so ProB can explore missing-binding and missing-registry cases. Tightening that
scope without losing the interesting cases is follow-up work.
