...
\begin{zed}
  STATUSRESULT ::= statusOK | statusNoSession | statusUnknownSession
\end{zed}
...
\begin{zed}
  UNLOADRESULT ::= unloadOK | unloadActiveSession | unloadUnknownSession
\end{zed}
...
Status is observation, not progression. It reports known active or stopped
sessions. Unknown or unloaded sessions are not statusable.

\begin{schema}{StatusAllOK}
  \Xi Circuit \\
  result! : STATUSRESULT
\where
  sessions \neq \emptyset \\
...
\begin{schema}{StatusAllEmpty}
  \Xi Circuit \\
  result! : STATUSRESULT
\where
  sessions = \emptyset \\
  result! = statusNoSession
\end{schema}

\begin{zed}
  StatusAll \defs StatusAllOK \lor StatusAllEmpty
\end{zed}

\begin{schema}{StatusTargetedOK}
  \Xi Circuit \\
  s? : SESSION \\
  result! : STATUSRESULT
\where
...
\begin{schema}{StatusTargetedUnknown}
  \Xi Circuit \\
  s? : SESSION \\
  result! : STATUSRESULT
\where
  s? \notin sessions \\
  result! = statusUnknownSession
\end{schema}

\begin{zed}
  StatusTargeted \defs StatusTargetedOK \lor StatusTargetedUnknown
\end{zed}

% ============================================================
\section{Stop, Unload, and Context Injection}
% ============================================================
...
\begin{schema}{UnloadActiveError}
  \Xi Circuit \\
  s? : SESSION \\
  result! : UNLOADRESULT
\where
  s? \in active \\
  result! = unloadActiveSession
\end{schema}

\begin{schema}{UnloadUnknown}
  \Xi Circuit \\
  s? : SESSION \\
  result! : UNLOADRESULT
\where
  s? \notin sessions \\
  result! = unloadUnknownSession
\end{schema}

\begin{zed}
  UnloadTargeted \defs UnloadStoppedOK \lor UnloadActiveError \lor UnloadUnknown
\end{zed}
...
Status stopped & stopped sessions remain inspectable \\
Unload stopped & stopped sessions are removed from storage \\
Unload active & active sessions cannot be unloaded \\
